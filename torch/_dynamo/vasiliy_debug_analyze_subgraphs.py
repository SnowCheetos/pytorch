...
def profile_performance(
    ...
    # print('logs_out', logs_out)
    # os.environ['TORCH_LOGS_OUT'] = logs_out
    ...
    * benchmark fw+bw for each and record the runtime, display a table comparing
    ...
    for row_idx, row in enumerate(summary_rows[1:]):
        ...
        if bench_compile:
            reset_memory()
            # save torch.compile logs to a file specific to this benchmark run
            # orig = os.environ.get('TORCH_LOGS')
            # os.environ['TORCH_LOGS'] = 'output_code'
            # new = os.environ.get('TORCH_LOGS')
            torch._logging.set_logs(output_code=True)
            # print(orig, new, orig == new)
            logs_out = os.path.join(target_folder, f'torch_logs_{subgraph_idx}_compile.txt')
            # print('logs_out', logs_out)
            # os.environ['TORCH_LOGS_OUT'] = logs_out
            m_c = torch.compile(m)
