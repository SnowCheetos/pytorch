    ...
    try:
        ...
        if "mismatch" in e.args[0]:
            ...
            def _compare(tree, dynamic_shapes, path):
                ...
                if tree.context != dynamic_shapes.context:
                    raise_mismatch_error(
                ...
                for i, (tree_, dynamic_shapes_) in enumerate(
                    zip(tree.children_specs, dynamic_shapes.children_specs)
                ):
                    _compare(
                        tree_, dynamic_shapes_, path + [_key(tree.type, tree.context, i)]
                    )
...
def _process_dynamic_shapes(
            ...
            for i, dim in shape.items():
                if isinstance(dim, (int, _Dim)):
                    ...
                    if dim is not None:
                        raise UserError(
                            UserErrorType.INVALID_INPUT,
                            f"Unexpected dimension mapped to index {i} in input tensor shape {shape} specified at `dynamic_shapes{keystr(path)}` "
            ...
            for i, dim in enumerate(shape):
                if isinstance(dim, (int, _Dim)):
                    ...
                    if dim is not None:
                        raise UserError(
                            UserErrorType.INVALID_INPUT,
                            f"Unexpected dimension #{i} in input tensor shape {shape} specified at `dynamic_shapes{keystr(path)}` "
                            f"(expected None, an int, or a Dim, but got {dim} instead)",
                        )
        else:
    ...
    if isinstance(dynamic_shapes, dict):
        ...
        if got_keys != expected_arg_names:
            ...
            if (
                len(combined_args) == 1 and
                expected_arg_names[0] not in got_keys and
                isinstance(combined_args[expected_arg_names[0]], dict)
            ):
                msg += (
            ...
            raise UserError(UserErrorType.INVALID_INPUT, msg)
    else:
